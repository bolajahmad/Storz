...
function Home() {
  ...
  return (
    ...
    >
      <Toaster />
      <img
        src={gradient_triangle}
        alt=""
        className={styles["gradient-triangle"]}
      />
      <img src={glass_triangle} alt="" className={styles["glass-triangle"]} />
      <img
        src={right_gradient_triangle}
        alt=""
        className={styles["right-gradient-triangle"]}
      />
      <img
        src={right_glass_triangle}
        alt=""
        className={styles["right-glass-triangle"]}
      />
      <div className={styles["home-header"]}>
        ...
        <div className={styles["home-desc"]}>
          {isUploading
            ? "Uploading your files to our IPFS network..."
            : success
            ? "All your files have been encrypted and uploaded to our IPFS network successfully!"
            : "Upload your files securely to our IPFS network"}
        </div>
      </div>

      {!isUploading && !success ? (
        <div className={styles["upload-box-con"]}>
          <div className={styles["upload-box"]} {...getRootProps()}>
            <input {...getInputProps()} />
            <img src={upload_icon} alt="upload icon" />
            Drop your files here
          </div>
          <div className={styles["glass-row-con"]}>
            {files?.map((file, idx) => {
              return (
                <div className={styles["glass-row"]} key={idx}>
                  <div className={styles["glass-file-name"]}>{file.name}</div>
        ...
        <div className={styles["upload-spinner-con"]}>
          <img src={uploading_spinner} alt="uploading spinner" />
        </div>
      ) : (
        <div className="upload-success-con">
      ...
      {!isUploading && !success && files && files.length > 0 ? (
        <div
          className={styles["upload-btn"]}
          onClick={() => createFormData(files)}
        >
          <p>Upload</p>
        </div>
      ) : (
